...
function FavouritesView({ onBack }) {
  ...
  useEffect(() => {
    ...
    async function loadFriends() {
      try {
        ...
        const nextFriends = await response.json();
        if (isCurrent) {
        ...
        if (isCurrent) {
          setError('Favourites are temporarily unavailable.');
          setIsLoading(false);
        }
      }
    }
    loadFriends();
    const refresh = window.setInterval(loadFriends, 5000);
    return () => {
      isCurrent = false;
  ...
  async function addItem(event) {
    ...
    if (!name || !activeFriend) return;
    const response = await fetch('/api/favourites', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'item', friendId: activeFriend.id, name, type: itemType }),
    });
    if (!response.ok) return setError('Could not save this item.');
    const item = await response.json();
    setFriends((currentFriends) => currentFriends.map((friend) => (friend.id === activeFriend.id ? { ...friend, items: [...friend.items, item] } : friend)));
    setItemName('');
  }
  ...
  async function addFriend(event) {
    ...
    const response = await fetch('/api/favourites', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'friend', name }),
    });
    if (!response.ok) return setError('Could not add this friend.');
  ...
  async function removeItem(itemId) {
    const response = await fetch('/api/favourites', {
      method: 'DELETE',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ friendId: activeFriend.id, itemId }),
    });
    if (!response.ok) return setError('Could not remove this item.');
    ...
    <div className="min-h-screen px-4 py-6 text-slate-100 sm:px-8 sm:py-10">
      <div className="mx-auto max-w-6xl">
        <header className="flex flex-col gap-5 border-b border-white/15 pb-8 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="rounded-lg border border-cyan-300/30 bg-cyan-300/10 px-4 py-3 text-sm text-cyan-100">{friends.reduce((total, friend) => total + friend.items.length, 0)} items shared</div>
        </header>
        ...
        <main className="mt-8 grid gap-6 lg:grid-cols-[240px_1fr]">
          ...
          <section className="rounded-xl border border-white/10 bg-slate-950/65 p-5 shadow-2xl sm:p-7">
            {error && <p className="mb-4 rounded-md border border-red-300/30 bg-red-300/10 px-3 py-2 text-sm text-red-200">{error}</p>}
            {isLoading ? <p className="py-12 text-center text-slate-400">Loading shared favourites...</p> : !activeFriend ? <p className="py-12 text-center text-slate-400">No favourites tabs are available.</p> : <>
